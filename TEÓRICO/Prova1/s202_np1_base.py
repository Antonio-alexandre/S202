...
class Neo4jDriver:
    neo4j_host = "neo4j+s://31e5ab0b.databases.neo4j.io"
    neo4j_user = "neo4j"
    neo4j_password = "FBYJ47SklnwCeJadAR6B2YHFhzzP9WoRzcSoayG6aR8"
...
class ItemDAO:
    ...
    def get_available_itens(self, capacity, coins):
        ...
        items = self.mongo_collection.find(query)
        sorted_items = sorted(items, key=lambda x: (x['price'], x['weight']))
        return [(item['name'], item['price']) for item in sorted_items]
...
class PageDAO:
    ...
    def add_page(self, page : Page):
        with self.neo4j_driver.session() as session:
            session.run("""
            CREATE (p:Page {title: $title, description: $description, characteristic: $characteristic})
        """, page.to_dict())
...
class CharacterDAO:
    ...
    def get_knowledge(self, character_name):
        with self.neo4j_driver.session() as session:
            result = session.run("""
                MATCH (c:Character {name: $name})-[:KNOWS]->(p:Page {characteristic: $profession})
